        ...
        try:
            ...
            if Banco.cursor.fetchone() is not None:
                ...
                def Pesagem():
                    lPs=Tk()
                    lPs.title('PESAGEM')
                    lPs.geometry('250x200')
                    lPs.configure(background='#dde')
                    lPs.bind("<Escape>", lambda x: lPs.destroy())

                    def avancar():
                        lPs.destroy()
                        Pedido()

                    bAtualizar=Button(lPs, text='Atualizar (F5)', width=8, command=lambda:[avancar()])
                    lPs.bind("<F5>", lambda x:[avancar()])
                    bAtualizar.place(x=5, y=5, width=100, height=30)
